...
const Navbar = () => {
  ...
  return (
    <div className="w-full h-16 fixed top-0 left-0 z-50 mb-200px bg-white">
      <div className="md:flex items-center justify-between py-4 md:px-10 px-7">
        ...
        >
          {links.map((link, i) => (
            <m.div
              key={link}
              className="md:ml-8 p-1 md:my-0 my-7 text-gray-900 bg-white border hover:border-gray-300 focus:outline-none hover: font-medium rounded-full text-sm px-5 py-2.5 mr-2 mb-2 hover:translate-y-1 transition duration-500 ease-in-out"
              initial={{
                opacity: 0,
              }}
              animate={{ opacity: 1 }}
